...
public partial class EconomyUi : Control
{
	...
	private bool sellButtonConnected = false;
	...
	private void FindSellEnergyButton()
	{
		...
		)
		{
			...
			string text =
				button.Text.Trim().ToLowerInvariant();

			string nodeName =
				button.Name.ToString().Trim().ToLowerInvariant();
			...
			)
			{
				...
				if (!sellButtonConnected)
				{
					sellEnergyButton.Pressed +=
						OnSellEnergyPressed;
	...
	private void UpdateDisplay()
	{
		...
		EnergySystem economy =
			EnergySystem.Instance;

		energyLabel.Text =
			"Energy: " +
			economy.Energy.ToString("F2");

		dollarsLabel.Text =
			"Dollars: $" +
			economy.Dollars.ToString("F2");

		if (sellEnergyButton != null)
		{
			sellEnergyButton.Disabled =
				economy.Energy <
				EnergySystem.EnergyPerDollar;
		}
	}
}
