...
public partial class WheelUpgradeUi : Control
{
    ...
    {
        if (inputBlockedByPurchaseWindow) return;
        if (simulator == null || !simulator.IsWheelUnlocked(wheelIndex)) return;
        if (!simulator.HasAvailableWheelUpgrades(wheelIndex)) return;
    ...
    {
        if (upgradeWindow != null && IsInstanceValid(upgradeWindow)) upgradeWindow.QueueFree();
        upgradeWindow = null;
    }
    ...
    public void SetPurchaseModalInputBlocked(bool blocked)
    {
        ...
        {
            if (buttons[i] != null) buttons[i].MouseFilter = blocked ? Control.MouseFilterEnum.Ignore : Control.MouseFilterEnum.Stop;
            if (windows[i] != null) windows[i].MouseFilter = blocked ? Control.MouseFilterEnum.Ignore : Control.MouseFilterEnum.Stop;
        }
    }
    ...
    private bool CanPurchaseAnyUpgrade(int wheelIndex)
    {
        ...
        {
            if (simulator.CanPurchaseWheelUpgrade(wheelIndex, types[i])) return true;
        }
        return false;
    }
    ...
    private void Refresh()
    {
        ...
        for (int wheelIndex = 0; wheelIndex < MaxWheelCount; wheelIndex++)
        {
            ...
            bool hasUpgrade = simulator.IsWheelUnlocked(wheelIndex) && simulator.HasAvailableWheelUpgrades(wheelIndex);
            panel.Visible = hasUpgrade;
            if (!hasUpgrade) continue;

            Vector2 wheelPosition = simulator.GetWheelUiPosition(wheelIndex);
            panel.Position = new Vector2(wheelPosition.X - WindowWidth * 0.5f - 4.0f, wheelPosition.Y - WindowHeight * 0.5f);

            if (buttons[wheelIndex] != null)
            {
                bool affordableUpgradeExists = CanPurchaseAnyUpgrade(wheelIndex);
                ApplyButtonStyle(buttons[wheelIndex], affordableUpgradeExists);
                buttons[wheelIndex].MouseFilter = inputBlockedByPurchaseWindow ? Control.MouseFilterEnum.Ignore : Control.MouseFilterEnum.Stop;
            }
